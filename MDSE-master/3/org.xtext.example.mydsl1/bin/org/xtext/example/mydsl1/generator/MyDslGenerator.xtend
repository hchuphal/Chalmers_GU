...
fsa.generateFile('LegoBrick.gv',
            '''  
            	...
            	«FOR m : resource.allContents.filter(ComplexBrick).toIterable»
                 ...
                 «ENDIF»   
                 //Top Line
                 «FOR x : 0 ..< m.getSize.get(i).dimensions.brickWidth»
                	//bottom wall
                	cube = new Cube(2.9, 2, 0.25);
                	pos = new Vec3(«((2.75*x)+0.75)+xAxis», «1.4+zAxis»,«yAxis-0.6» );
                	script.addObject(cube, new CoordinateSystem(pos, 0, 0, 0));
                	
                	//top wall
                	cube = new Cube(2.9, 2, 0.25);
                	pos = new Vec3(«((2.75*x)+0.75)+xAxis»,«1.4+zAxis», «((2.75*m.getSize.get(i).dimensions.brickHeight)-0.6)+yAxis»);
	                script.addObject(cube, new CoordinateSystem(pos, 0, 0, 0));
                 «ENDFOR»
                 «FOR y : 0 ..< m.getSize.get(i).dimensions.brickHeight»
                 //left wall
                 cube = new Cube(0.25, 2, 2.9);
                 pos = new Vec3(«xAxis-0.6», «1.4+zAxis»,«(2.75*y)+0.85+yAxis»);
                 script.addObject(cube, new CoordinateSystem(pos, 0, 0, 0));
                 
                 //Right wall
                 cube = new Cube(0.25, 2, 2.9);
                 pos = new Vec3(«(2.75*m.getSize.get(i).dimensions.brickWidth)-0.6», «1.4+zAxis»,«(2.75*y)+0.85+yAxis»);
                 script.addObject(cube, new CoordinateSystem(pos, 0, 0, 0));
